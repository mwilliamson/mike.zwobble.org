...
var buildNewParameters = function() {
    var parameters = inject.newInjector();
    var nameParameter = function(parameter) {
        return "parameters.id." + parameter.id;
    };
    var self = {
        add: function(parameter, value) {
            parameters.bind(nameParameter(parameter)).toConstant(value);
        },
        get: function(parameter, callback) {
            var key = nameParameter(parameter);
            if (parameters.isBound(key)) {
                parameters.get(key, function(err, value) {
                    callback(err, {status: status.ok, value: value});
                });
