...
var authenticateOnHttps = function(subMatcher) {
    return {
        match: function(components, parameterValues, callback) {
            var authorisationController = {
                name: "authorisation"
            };
            subMatcher.match(components, parameterValues, {
                ok: function(controllers) {
                    callback.ok([authorisationController].concat(controllers));
                },
                unmatched: callback.unmatched,
                notFound: function(value) {
                    callback.ok([authorisationController]);
                },
...
var navigatingMiddleware = function(request, response, next) {
    ...
    navigator.navigate(path, function(err, result) {
        if (err) {
            ...
            var controllers = result.value;
            async.forEachSeries(controllers, function(controller, next) {
