var connect = require("connect"),
    ...
    gzipConnect = require("connect-gzip");
...
var pageNumberParameter = convert(parameters.regex(/([0-9]+)/), toInt);
...
injector.bind("mongoDb").toProvider(mongo.connect);
...
injector.bind("uncachedTopicRepository").toProvider(topics.topicRepository, "mongoDb");
injector.bind("topicRepository").toProvider(topics.cachedTopicRepository, "uncachedTopicRepository").memoize();
...
injector.bind("controllers.feed").toSyncProvider(require("./controllers/feed"), "content", "publicPostRepository");
...
injector.bind("controllers.old.feed").toSyncProvider(oldControllers.feed, "content");
...
injector.bind("controllers.adminExport")
    .toSyncProvider(require("./controllers/adminExport"), "adminPostRepository", "topicRepository");
        ...
        match: function(components, parameterValues, callback) {
            ...
            callback.ok({
                ...
                next: function(callback) {
                    subMatcher.match(components, parameterValues, callback);
                }
            });
        }
    };
...
var navigatorPaths = paths.firstOf(
    ...
    then("page",
        then(pageNumberParameter,
            end(useController("frontPage", pageNumberParameter))
        )
    ),
    then(yearParameter,
...
var navigatingMiddleware = function(request, response, next) {
    ...
    var executeController = function(err, result) {
        ...
        if (err) {
            ...
            respond.next = function() {
                if (controller.next) {
                    controller.next(paths.useStandardCallback(executeController));
                } else {
                    executeController(null, {name: "http404"});
                }
            };
    ...
    navigator.navigate(path, executeController);
};
