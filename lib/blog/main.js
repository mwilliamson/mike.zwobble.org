...
var authenticateOnHttps = function(subMatcher) {
    // TODO: remove tri-state status, and simply use a Boolean isMatched
    //       instead of status.notFound, return isMatched=true with a 404 controller
    return {
        match: function(components, parameterValues, callback) {
            subMatcher.match(components, parameterValues, function(err, result) {
                if (err) {
                    ...
                    callback(null, {status: result.status, value: {
                        name: "authorisation",
                        parameterValues: [result.value]
                    }});
                }
            });
        }
