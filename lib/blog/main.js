...
// TODO: Should use as provider with singleton scope
require("./mongo").connect(function(err, db) {
    // TODO: handle err
    if (err) {
        require("sys").puts(err);
        throw err;
    } else {
        injector.bind("mongoDb").toConstant(db);
    }
});
injector.bind("postRepository").toProvider(require("./posts").publicPostRepository, "mongoDb");
injector.bind("controllers.frontPage").toSyncProvider(require("./controllers/frontPage"), "postRepository");
...
var postParameter = parameters.composite(
    ...
    function(year, month, slug, callback) {
        injector.get("postRepository", function(err, postRepository) {
