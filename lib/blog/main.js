...
var buildParameterCallback = function(callback) {
    return function(err, value) {
        if (err) {
            ...
            callback(null, {status: parameters.status.ok, value: value});
        }
    };
};
...
var navigatingMiddleware = function(navigatorPaths) {
    return function(request, response, next) {
        ...
        navigator.navigate(path, function(err, result) {
            ...
            if (err) {
                ...
                controllerInjector.get("controllers." + controller.name, function(err, controller) {
                    if (err) {
                        errorController(500, err, respond);
                    } else {
...
// TODO: don't split paths between HTTP and HTTPS, force user onto HTTPS for /admin
// TODO: only require HTTP auth for /admin

connect.createServer(
    staticMiddleware,
    navigatingMiddleware(httpPaths)
...
connect.createServer(
    ...
    connect.basicAuth(function(username, password, callback) {
        if (username !== settings.auth.username) {
            callback(null, false);
        } else {
            bcrypt.compare(password, settings.auth.hash, callback);
        }
    }),
    staticMiddleware,
    connect.bodyParser(),
    navigatingMiddleware(httpsPaths)
